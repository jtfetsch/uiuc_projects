...
static void put_ms(const std::string& message, int sockfd, sockaddr_storage their_addr, int addr_len);
...
bool rebalance_files()
{
	...
	for (auto iter = unbalancedFiles.begin(); iter != unbalancedFiles.end(); ++iter)
	{
		vector<ID> recipients = getIDsToPutFilesToByFile(iter->count==1, iter->name);
		// PUTS#ID_to_put_to_1#ID_to_put_to_2#ID_to_put_to_3#sdfsfilename#localfilename
		stringstream ss;
		ss << "PUTS";
		...
		ss << "#" << iter->name << "#" << iter->name;
		if(send_message(iter->source, mapping[iter->source], ss.str()))
			fileList[it->source].push_back(it->name);
	}
	return unbalancedFiles.size()>0;
}
	...
	{ // main accept() loop
		...
		// PUT (from not master): master determines who to put file to
			...
			if (!fork())
			{
				...
				send_message_no_ack(send_to_id, mapping[send_to_id], ss.str()); // could do this with reply message too
				for (auto it = IDs_to_put_to.begin(); it != IDs_to_put_to.end(); ++it)
				{
					fileList[*it].push_back(splitResults[2]); 
				}
				send_help_messages();
				exit(0);
			}
		...
		// PUT (for sdfs maintainance): contains locations of 3 nodes that the file should go to (use ftpClient to distribute)
		else if (prefix.compare("PUTS") == 0) {
			if (!fork())
			{
				put_ms(bufst, sockfd, their_addr, addr_len);
				exit(0);
			}
		}
...
static void put_m(const std::string& message, int sockfd, sockaddr_storage their_addr, int addr_len)
{
	...
	std::cout << "#########" << sdfsFileName << "\n";
	std::cout << "#########" << localFileName << "\n";
	for (int i = 0; i < numIds; ++i)
	{
		const ID node_id = nodeIds[i];
		std::string hostName = node_id.ip();
		std::cout << "ftpClient::send_put(" << hostName << "," << localFileName << "," << sdfsFileName << ")\n";
