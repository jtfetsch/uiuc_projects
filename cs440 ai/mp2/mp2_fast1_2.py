...
def selectHint(state):
    ...
    for pos in state.hintPositions:
        ...
        valid = [len(x) for x in state.vars if posVal in x]
        if len(valid) == 0:
            continue
        thisMaxWordLen = max(valid)
...
def orderDomainValues(word, state, (hintVal, hintCount)):
    ...
    # if len(sortedValues) == 0: # If we have negative values, they must be assigned
...
def solveSudoku(state):
    ...
    EXPANDED_COUNT += 1
    # Select a variable
    var = selectUnassigned(state, hint)
    if not var:
        return False
