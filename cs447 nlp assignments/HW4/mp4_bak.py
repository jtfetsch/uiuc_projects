...
Original file is located at
    https://colab.research.google.com/drive/11cBlHR_8lW8k7Wc5s7CszOl52j-n88aZ
...
class Encoder(nn.Module):
    ...
    def forward(self, x, lens):
        ...
        #lens = [batch size]
        embeds = self.embeds(x.long()) # torch.Size([max_len, batch_size, embedding_dim])

        ####################################################
        # TODO rotate embedding vector to be batch first?
        #embeds = embeds.permute(1, 0, 2) # torch.Size([batch_size, max_len, embedding_dim])
        # TODO rotated embedding vector works?
        ####################################################

        embeds = nn.utils.rnn.pack_padded_sequence(embeds, lens, enforce_sorted=False).float()
        embeds.to(device)
        ...
        lstm_out, out_lens = nn.utils.rnn.pad_packed_sequence(lstm_out, total_length=self.max_len)
        #hidden = [n_layers*num_directions, batch_size, hidden_dim]
        #c = [n_layers*num_directions, batch_size, hidden_dim]
        #lstm_out = [MAX_LEN, batch_size, hidden_dim*num_directions]
...
class Decoder(nn.Module):
    def __init__(self, vocab_size, embedding_dim, dec_units, enc_units, batch_sz):
        ...
        self.gru = nn.GRU(input_size = embedding_dim, hidden_size = HIDDEN_DIM*num_directions, num_layers = NUM_LAYERS, dropout = self.dropout_val, batch_first=True)
        self.linear = nn.Linear(HIDDEN_DIM*num_directions, vocab_size)
        self.softmax = nn.LogSoftmax(dim=1)
        self.dropout = nn.Dropout(self.dropout_val)

        
        self.W_a = nn.Linear(self.hidden_dim * num_directions, self.hidden_dim, bias=False)
        self.W_c = nn.Linear(self.hidden_dim * num_directions + self.hidden_dim, self.hidden_dim, bias=False)
        
        self.W_proj = nn.Linear(self.hidden_dim, self.embedding_dim, bias=False)
        self.W_s = nn.Linear(self.embedding_dim, self.vocab_size, bias=False)
        self.W_s.weight = self.embeds.weight
    ...
    def forward(self, x, decoder_hidden, enc_output, src_lens):
        ...
        '''
        x          : [60, 1]
        enc_output : [16, 60, 256]
        hidden[0]  : [4, 60, 256]
        hidden[1]  : [4, 60, 256]
        '''

        #x = [MAX LEN, batch size]
        #lens = [batch size]
        ...
        # pass input through recurrent decoder net
        ...
        ##############################################################
        ##############################################################
        # attention vector meant to be used before lstm usage - without it we aren't actually considering anything from those values!!!
        ##############################################################
        ##############################################################
        ...
        decoder_out = self.softmax(self.linear(decoder_out[0]))

        '''
        print("Decoder output is of shape", decoder_out.shape)
        print("Decoder hidden state is of shape", decoder_hidden[0].shape)
        print("Decoder c state is of shape", decoder_hidden[1].shape)
        print("Decoder lens is of shape", src_lens.shape)
        print(src_lens)
        ...
        #decoder_out, attention_weights = self.calculate_attention_vectors(decoder_out, enc_output, src_lens, len(enc_output))
        attention_weights = None
        return decoder_out, decoder_hidden, attention_weights

    def calculate_attention_vectors(self, decoder_output, encoder_outputs, src_lens, max_len):
        """
        Calculating the attention weights is done with another feed-forward layer attn, using the decoder’s input and hidden state as inputs. 
        Because there are sentences of all sizes in the training data, to actually create and train this layer we have to choose 
        a maximum sentence length (input length, for encoder outputs) that it can apply to. 
        ...
        attention_mask = self.sequence_mask(src_lens, max_len).unsqueeze(1)
